...
class GoogleCalendarService
{
    ...
    protected function authorizedRequest(string $method, string $url, array $options = []): ?array
    {
        ...
        $successStatuses = $options['success_statuses'] ?? [];
        unset($options['success_statuses']);
        ...
        try {
            ...
            if ($statusCode >= 400 && !in_array($statusCode, $successStatuses, true)) {
     ...
     * The email is fetched from Google's userinfo endpoint on first call
     * and cached inside the stored token file to avoid repeated API calls.
     */
    public function getConnectedEmail(): ?string
    {
        ...
        // Return cached email if already stored in the token payload.
        if (!empty($token['email'])) {
            return $token['email'];
        }

        // Fetch from Google userinfo API and cache it.
        try {
            $response = $this->sendRequest('GET', 'https://www.googleapis.com/oauth2/v3/userinfo', [
                'headers' => [
                    'Authorization' => 'Bearer ' . $token['access_token'],
                    'Accept' => 'application/json',
                ],
            ...
            if ($response->getStatusCode() < 400 && !empty($payload['email'])) {
            ...
            Log::warning('Google userinfo fetch failed.', ['message' => $e->getMessage()]);
        }
