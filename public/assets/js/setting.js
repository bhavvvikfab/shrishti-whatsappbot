...
function whatsappAjaxOptions(options) {
    var settings = $.extend(true, {
        ...
        beforeSend: function (xhr) {
            if (typeof window.crmAuthToken === 'function') {
                var token = window.crmAuthToken();
                if (token) {
    ...
    if (typeof window.crmApplyAuthHeaders === 'function') {
        settings.headers = window.crmApplyAuthHeaders(settings.headers || {});
    }
...
$(document).ready(function () {
    var settingsConfig = window.settingsPageConfig || {};
    var whatsappModuleEnabled = settingsConfig.whatsappModuleEnabled !== false;

    // when WhatsApp tab is shown, load existing config from database
    ...
    // load when page opens directly on #whatsapp-configure (hash may activate tab before handler binds)
    if (whatsappModuleEnabled && window.location.hash === '#whatsapp-configure') {
        setTimeout(maybeLoadWhatsappConfig, 0);
    }
...
function maybeLoadWhatsappConfig() {
    var pane = document.getElementById('whatsapp-configure');
...
function loadWhatsappConfig() {
    var settingsConfig = window.settingsPageConfig || {};
    if (settingsConfig.whatsappModuleEnabled === false) {
        $('#wa_status_msg').text('');
        return;
    }
...
function saveWhatsappConfig() {
    ...
    $.ajax(whatsappAjaxOptions({
        ...
        data: {
            ...
            verify_token: $('#wa_verify_token').val()
        },
        success: function () {
